...
Module -> Result<Top, ()>
  : PubOption 'module' 'ID' '{' Program '}'
    { Ok(Top::ModDecl{ span : $span, public : $1?, name : $3, body : $5? }) };

PackageName -> Result<PackageName, ()>
  : 'ID'                  { Ok(vec![$1]) }
  | PackageName '::' 'ID' { flatten($1, $3) }
  ;

PackageId -> Result<ImportName, ()>
  : PackageName           { Ok(($1?, None)) }
  | PackageName '::' '*'  { Ok(($1?, Some($3.span()))) }
  ;
...
TypeVar -> Result<TypeVar, ()>
  : 'ID'          { Ok(TypeVar{ span : $span, name : $1, kind : Kind::Type }) }
...
FuncDecl -> Result<Top, ()>
  : PubOption 'fn' 'ID' TypeVars '(' Arguments ')' Stmt
      { Ok(Top::FuncDecl{ span : $span, public : $1?, attr : None(), name : $3?, tyVars : $4?,
                          args : $6?, ty : None(), body : $8? }) }
  | 'FUNC_ATTR' PubOption 'fn' 'ID' TypeVars '(' Arguments ')' Stmt
      { Ok(Top::FuncDecl{ span : $span, public : $2?, attr : Some($1), name : $4?, tyVars : $5?,
                          args : $7?, ty : None(), body : $9? }) }
  | PubOption 'fn' 'ID' TypeVars '(' Arguments ')' ':' Type Stmt
      { Ok(Top::FuncDecl{ span : $span, public : $1?, attr : None(), name : $3?, tyVars : $4?,
                          args : $6?, ty : Some($9?), body : $10? }) }
...
Stmt -> Result<Stmt, ()>
  ...
  | 'if' '(' Expr ')' Stmt 'else' Stmt
      { Ok(Stmt::IfStmt{ span : $span, cond : $3?, thn : Box::new($5?), els : Some($7?) }) }
  | 'match' '(' Expr ')' Cases
...
Expr -> Result<Expr, ()>
  ...
  | Expr 'DOT_NUM'
      { Ok(Expr::NumField{ span : $span, lhs : Box::new($1?), rhs : $2 }) }
  | Expr '[' Exprs ']'
  ...
  | IntLit
      { Ok(Expr::IntLit{ span : $1?.0, base : $1?.1 }) }
  | 'FLOAT'
...
#[derive(Debug)]
pub struct Case { span : Span, patSpan : Span, pat : Vec<Pattern>, body : Stmt }

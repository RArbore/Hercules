...
pub fn print_token<'a>(t : &Token, l : &'a Lexer) -> &'a str {
    match &t.tok {
        TokenType::As           => "as",
        TokenType::By           => "by",
        ...
        TokenType::EqualOrs     => "||=",
        TokenType::EqualPlus    => "+=",
        TokenType::EqualShiftL  => "<<=",
        TokenType::EqualShiftR  => ">>=",
        TokenType::EqualTimes   => "*=",
        TokenType::EqualXor     => "^=",
        TokenType::Equals       => "==",
        TokenType::EqualsNot    => "!=",
        TokenType::Greater      => ">",
        ...
        TokenType::Error(e)     =>
            match e {
                LexError::AttrEOF => "Lexer Error: Function attribute not closed before EOF",
                LexError::AttrEOL => "Lexer Error: Function attribute not closed before EOL",
                LexError::CommentEOF => "Lexer Error: Block comment not closed before EOF",
                LexError::HexEmpty => "Lexer Error: Incomplete hexadecimal literal",
                LexError::HexError => "Lexer Error: Invalid hexadecimal literal",
                LexError::BinEmpty => "Lexer Error: Incomplete binary literal",
                LexError::BinError => "Lexer Error: Invalid binary literal",
                LexError::OctEmpty => "Lexer Error: Incomplete octal literal",
                LexError::OctError => "Lexer Error: Invalid octal literal",
                LexError::FloatExpEmpty => "Lexer Error: Empty float literal exponent",
                LexError::InternalError => "PANIC: Internal Lexer Error",
            },
    }
}
...
impl Lexer {
    ...
    pub fn get_line(&self, lineno : usize) -> &str {
        if lineno == self.lines.len() && lineno > 0 {
            let (start_prev, len_prev) = self.lines[lineno-1];
            let start_line = start_prev + len_prev;
            match self.contents[start_line..].find('\n') {
                Some(idx) => &self.contents[start_line..start_line+idx+1],
                None      => &self.contents[start_line..],
            }
        } else if self.lines.is_empty() {
            match self.contents.find('\n') {
                Some(idx) => &self.contents[..idx+1],
                None      => &self.contents,
            }
        } else if lineno > self.lines.len() {
            "<Failed To Read Line From Lexer>\n"
        } else {
            let (start, len) = self.lines[lineno];

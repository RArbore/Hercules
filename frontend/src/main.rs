...
fn unparse_kind(
    k : &Kind,
    _lexer : &dyn NonStreamingLexer<DefaultLexerTypes<u32>>
) -> String {
    match k {
...
fn unparse_top(
    ...
    match t {
        Top::Import{ span : _, name }
            => format!("{}use {};",
                       indentation(indent),
                       unparse_import_name(name, lexer)),
        Top::TypeDecl{ span : _, public, name, ty_vars, body }
            => unparse_type_decl(public, name, ty_vars, body, indent, lexer),
        Top::ConstDecl{ span : _, public, name, ty, body }
            => unparse_const_decl(public, name, ty, body, indent, lexer),
        Top::FuncDecl{ span : _, public, attr, name, ty_vars, args, ty, body }
            => format!("{}{}{}fn {}{}({}){} {}",
                       indentation(indent),
...
fn unparse_type_decl(
    public : &bool, name : &Id, ty_vars : &Vec<TypeVar>, body : &TyDef,
    indent : usize,
    lexer : &dyn NonStreamingLexer<DefaultLexerTypes<u32>>
) -> String {
    format!("{}{}type {}{} = {};",
...
fn unparse_type_var(
    ...
    format!("{} : {}",
            unparse_id(&ty_var.name, lexer),
            unparse_kind(&ty_var.kind, lexer))
}
...
fn unparse_type_def(
    ...
    match def {
        TyDef::TypeAlias{ span : _, body } => unparse_type(body, lexer),
        TyDef::Struct{ span : _, public, fields } => {
            format!("{}struct {{ {} }}",
                    if *public { "pub " } else { "" },
                    unparse_obj_fields(fields, lexer))
        },
        TyDef::Union{ span : _, public, fields } => {
            format!("{}union {{ {} }}",
                    if *public { "pub " } else { "" },
                    unparse_obj_fields(fields, lexer))
        },
...
fn unparse_type_exprs(
    ...
    if ty_exp.is_empty() {
        ...
        format!("<{}>", ty_exp.iter().map(|e| unparse_type_expr(e, lexer))
                             .collect::<Vec<_>>().join(", "))
    }
}
...
fn unparse_lexpr(
    ...
    match exp {
        LExpr::VariableLExpr { span } => unparse_id(span, lexer),
        LExpr::FieldLExpr { span : _, lhs, rhs } =>
            format!("{}.{}", unparse_lexpr(lhs, lexer), unparse_id(rhs, lexer)),
        LExpr::NumFieldLExpr { span : _, lhs, rhs } =>
            format!("{}{}", unparse_lexpr(lhs, lexer), unparse_id(rhs, lexer)),
        LExpr::IndexLExpr { span : _, lhs, index } =>
...
fn unparse_pattern(
    ...
    match pat {
        Pattern::SimplePattern{ span : _, pat } =>
            unparse_spattern(pat, lexer),
        Pattern::UnionPattern{ span : _, name, pat } =>
            format!("{} {}", unparse_package_name(name, lexer),
                    unparse_spattern(pat, lexer)),
    }
}
...
fn unparse_spattern(
    ...
    match pat {
        ...
        SPattern::IntLit{ span, base } =>
            unparse_int_lit(span, base, lexer),
        SPattern::Variable{ span : _, name } =>
            unparse_package_name(name, lexer),
        SPattern::TuplePat{ span : _, pats } =>
            format!("({})", pats.iter().map(|p| unparse_pattern(p, lexer))
                                .collect::<Vec<_>>().join(", ")),
        SPattern::OrderedStructPat { span : _, pats } =>
            format!("{{{}}}", pats.iter().map(|p| unparse_pattern(p, lexer))
                                  .collect::<Vec<_>>().join(", ")),
        SPattern::NamedStructPat { span : _, pats } =>
            format!("{{{}}}", pats.iter()
                                  .map(|p| format!("{} = {}",
    ...
    match stmt {
        ...
        Stmt::ForStmt { span : _, var, init, bound, step, body } =>
            format!("{}for ({} = {} to {}{})\n{}", indentation(indent),
                    ...
                    unparse_stmt(body, indent+1, lexer)),
        Stmt::WhileStmt { span : _, cond, body } =>
            format!("{}while ({})\n{}", indentation(indent),
                    unparse_expr(cond, lexer),
                    unparse_stmt(body, indent+1, lexer)),
        Stmt::ReturnStmt { span : _, expr } =>
            format!("{}return {};", indentation(indent),
                    unparse_expr(expr, lexer)),
        Stmt::BlockStmt { span : _, body } =>
            format!("{}{{\n{}\n{}}}", indentation(indent-1),
                    body.iter().map(|s| unparse_stmt(s, indent, lexer))
                        .collect::<Vec<_>>().join("\n"),
                    indentation(indent-1)),
        Stmt::CallStmt { span : _, name, ty_args, args } =>
            format!("{}{}{}({});", indentation(indent),
                    unparse_package_name(name, lexer),
                    if ty_args.is_empty() { "".to_string() }
                    else { format!("::<{}>",
